...
- [شروع سریع](#شروع-سریع)
- [ساخت کانفیگ به‌صورت بصری](#ساخت-کانفیگ-بهصورت-بصری)
- [پیکربندی](#پیکربندی)
...
| سازندهٔ بصری کانفیگ | ندارد | رابط کاربری داخلی در مرورگر (`--config-ui`) |
| دستور اجرا | `snispf` | `snispf` **و** `snispf-hj` |
| ماژول‌های جدید | — | `pool.py`، `ip_discovery.py`، `config_server.py` |
...
## ساخت کانفیگ به‌صورت بصری

ویرایش دستی JSON برای همه راحت نیست. SNISPF-HJ یک **رابط کاربری بصری**
داخلی دارد که مستقیماً در مرورگرت باز می‌شود — بدون دانلود جدا، بدون
کپی‌کردن لینک.
...
# ذخیره مستقیم روی فایل کانفیگ فعلی‌ات
snispf-hj --config-ui --config my_config.json
```

این دستور یک سرور محلی کوچک (فقط روی `127.0.0.1` — هیچ‌وقت به شبکه expose
نمی‌شود) راه‌اندازی می‌کند و خودکار رابط کاربری را در مرورگر پیش‌فرضت باز
می‌کند. از آنجا می‌توانی:

- IP و SNI با دکمه‌های افزودن سریع اضافه/حذف کنی (لیست IP پیش‌فرض Cloudflare)
- پارامترهای pool، drain، eviction و discovery را با اسلایدر تنظیم کنی
- `config.json` خروجی را با رنگ‌بندی syntax پیش‌نمایش کنی
- دکمهٔ **«ذخیره در سرور»** را بزنی تا فایل مستقیم نوشته شود، یا
  **«دانلود»** برای ذخیرهٔ دستی

برای بستن سرور کانفیگ، `Ctrl+C` را در ترمینال بزن.
...
--config-ui               باز کردن رابط بصری ساخت کانفیگ در مرورگر
--ui-port PORT             پورت رابط کانفیگ (پیش‌فرض: 40080)
...
└── sni_spoofing/
    ...
    ├── config_server.py          # ★ رابط بصری داخلی ساخت کانفیگ
...
- **[@hjfisher](https://github.com/hjfisher)** — `CombinationExplorer`،
  ...
  drain timeout، حذف/قرنطینه/بازیافت IP، رابط بصری ساخت کانفیگ، و یکپارچه‌سازی
  کلی pool.
